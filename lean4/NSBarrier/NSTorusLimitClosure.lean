...
/-- T³-side bundle connecting the actual solution package to the verified compactness / limit
closure chain.

The actual torus solution package is kept explicit so that this file remains the bridge between the
PDE-facing branch and the abstract limit-side closure theorems. The remaining fields identify the
limit trajectories `Elim`, `Dlim` with the concrete state-level coefficient formulas used in
`NSCompactnessConvergenceActual`. -/
structure TorusLimitClosureData (K_max m : ℕ) where
  solutionPkg : NSTorusSolutionPackage.TorusActualSolutionPackage K_max m
...
/-- The concrete state-level compactness data yield pointwise convergence of the extracted
enstrophy trajectory appearing in the abstract limit-closure package. -/
theorem hElim_tendsto_of_torus_limit_data
    {K_max m : ℕ}
    (d : TorusLimitClosureData K_max m) :
    ∀ n : ℕ,
      Tendsto
        (fun j : ℕ =>
...
/-- Likewise, the concrete state-level compactness data yield pointwise convergence of the
low-shell dissipation trajectory used in the limit-closure package. -/
theorem hDlim_tendsto_of_torus_limit_data
    {K_max m : ℕ}
    (d : TorusLimitClosureData K_max m) :
    ∀ n : ℕ,
      Tendsto
...
/-- The torus-side compactness package induces the abstract limit-closure stability data. -/
noncomputable def toLimitClosureStabilityData_of_torus
    {K_max m : ℕ}
    (d : TorusLimitClosureData K_max m) :
  ...
  hElim_tendsto := hElim_tendsto_of_torus_limit_data d
  hDlim_tendsto := hDlim_tendsto_of_torus_limit_data d
  hDlow_le_CE := d.hDlow_le_CE

/-- B3 wrapper theorem: once the T³ compactness data are identified with the abstract limit-side
objects, the extracted torus limit inherits the same discrete closure / Grönwall law as the
Galerkin family. -/
theorem torus_limit_solution_inherits_closure
    {K_max m : ℕ}
    (d : TorusLimitClosureData K_max m) :
    ∀ n : ℕ,
  ...
  limit_closure_stability_master
    (toLimitClosureStabilityData_of_torus d)

end NSTorusLimitClosure
